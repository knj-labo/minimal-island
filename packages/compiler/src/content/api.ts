...
export function createContentManager(options: ContentManagerOptions): ContentAPI {
  ...
  const api: ContentAPI = {
    ...
    async getEntryBySlug<T = any>(collection: string, slug: string): Promise<ContentEntry<T> | null> {
      const entries = await loadCollection(collection);
...
export const queries = {
   ...
   * Sort by title
   */
  byTitle<T extends { data: { title?: string } }>(
    a: ContentEntry<T>,
    b: ContentEntry<T>
  ): number {
   ...
   */
  published<T extends { data: { draft?: boolean } }>(
